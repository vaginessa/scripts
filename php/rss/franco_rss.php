...
$changelog = file_get_contents('http://kernels.franco-lnx.net/Nexus5/5.1/appfiles/changelog.xml');
...
for($i=0;$i<10;$i++) {
    echo '<item>';
        ...
        echo '<link>http://kernels.franco-lnx.net/Nexus5/5.1/appfiles/changelog.xml</link>';
        echo '<description>';
          for($j=0; $j<count($versions->changelogversion[$i]->changelogtext); $j++) {
              echo $versions->changelogversion[$i]->changelogtext[$j] . "\n";
          }
        echo '</description>';
        echo '<guid>http://kernels.franco-lnx.net/Nexus5/5.1/appfiles/changelog.xml?'.$versions->changelogversion[$i]['versionName'].'</guid>';
    echo '</item>';
